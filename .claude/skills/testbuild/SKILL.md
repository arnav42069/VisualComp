---
name: testbuild
description: Build just the VisualComp 2.2 Standalone target, drop it into "Build Final\Standalone Test", and launch it for a fast test loop. Use when asked to "test build", "quick build", "build and run", or "/testbuild".
---
...
- Clear any stale Standalone settings first if a clean UI state is needed to
  ...
  `%APPDATA%\VisualComp 2.2\VisualComp 2.settings`.
