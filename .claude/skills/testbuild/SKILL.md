---
name: testbuild
description: Build just the VisualComp 2.38 Standalone target, drop it into "Build Final\Standalone Test", launch it, then commit and push the working tree to GitHub — a fast test-and-publish loop. Use when asked to "test build", "quick build", "build and run", or "/testbuild".
---
...
- Clear any stale Standalone settings first if a clean UI state is needed to
  ...
  `%APPDATA%\VisualComp 2.38\VisualComp 2.settings`.
