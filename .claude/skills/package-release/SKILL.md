...
description: Build VisualComp 2.2 (VST3 + Standalone) in Release and assemble the distributable zip via package.ps1. Use when asked to "package the release", "build the release zip", "cut a release build", or "/package-release".
---

# Package Release

Builds VisualComp 2.2 and assembles the Windows+Mac distributable zip in `Build Final\`.
...
2. **Run the packaging script**:
   ...
   ```
   - Assembles `Build Final\VisualComp 2.2 - Windows and Mac.zip` from whichever of `build\` / `build-pkg\` holds the newer `VisualComp 2.2.vst3` (the script checks both — see CLAUDE.md).
   - Bundles: Windows install/uninstall scripts + VST3 + standalone exe, the user manual PDF, and Mac source + build steps.
   - Also drops a loose, unzipped `Build Final\VisualComp 2.2.exe` (same binary as inside the zip) so the user can launch and test it directly without unzipping — this happens on every packaging run, not just on request.
...
- The version string lives in `package.ps1` (`$version = '2.2'`) — bump it there (and the CMake `PRODUCT_NAME`, per CLAUDE.md) before packaging a new version.
