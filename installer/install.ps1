...
# Launched by "Install VisualComp 2.37.bat", which handles the admin elevation.

$ErrorActionPreference = 'Stop'
$here = Split-Path -Parent $MyInvocation.MyCommand.Path

$vst3Src   = Join-Path $here 'VisualComp 2.37.vst3'
$exeSrc    = Join-Path $here 'VisualComp 2.37.exe'
$vst3Dest  = 'C:\Program Files\Common Files\VST3\VisualComp 2.37.vst3'
$appDir    = 'C:\Program Files\Azazel Audio\VisualComp 2.37'

Write-Host ''
Write-Host '  ================================================' -ForegroundColor DarkYellow
Write-Host '   VisualComp 2.37  -  Azazel Audio' -ForegroundColor Yellow
Write-Host '   See your compression. Shape your sound.' -ForegroundColor DarkGray
Write-Host '  ================================================' -ForegroundColor DarkYellow
Write-Host ''

if (-not (Test-Path -LiteralPath $vst3Src)) {
    Write-Host "  ERROR: 'VisualComp 2.37.vst3' not found next to this script." -ForegroundColor Red
...
if (Test-Path -LiteralPath $exeSrc) {
    ...
    Copy-Item -LiteralPath $exeSrc -Destination (Join-Path $appDir 'VisualComp 2.37.exe') -Force
...
try {
    ...
    $exePath = Join-Path $appDir 'VisualComp 2.37.exe'
    if (Test-Path -LiteralPath $exePath) {
        $shell = New-Object -ComObject WScript.Shell
        $lnk = $shell.CreateShortcut((Join-Path $startMenu 'VisualComp 2.37.lnk'))
        $lnk.TargetPath       = $exePath
        $lnk.WorkingDirectory = $appDir
        $lnk.Description      = 'VisualComp 2.37 - Azazel Audio'
        $lnk.Save()
        Write-Host "        -> Start Menu > Azazel Audio > VisualComp 2.37" -ForegroundColor DarkGray
    }
}
...
Write-Host '    2. The plugin appears as "VisualComp 2.37" by Azazel Audio (Dynamics).'
Write-Host ''
