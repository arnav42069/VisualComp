...
- Packaging is [`package.ps1`](package.ps1:1); it picks the newest output from [`build`](package.ps1:70) or [`build-pkg`](package.ps1:70), stages the release bundle, copies the standalone exe into the Windows folder inside the zip, and also copies a loose test exe into [`Build Final`](package.ps1:134).
